...
function testCanonicalTermsAssetMatchesPublicTerms() {
  ...
  expect(metadata.termsPublicPath === "https://github.com/JehyukJang/Tonigma-docs/blob/main/publication/project-wide-policy/legal/terms.md", "Unexpected public Terms URL.");
...
function assertAgentGuidance(payload, expectedRefs) {
  ...
  expect(
    payload.agentGuidance.termsSource === "https://github.com/JehyukJang/Tonigma-docs/blob/main/publication/project-wide-policy/legal/terms.md",
...
function testHelpCommandsOutputUsesFinalPromptPolicy() {
  ...
  expect(stdout.includes("Use --network to install only that network's deployment artifacts"), "Install help should explain network-scoped installs.");
  expect(stdout.includes("Mainnet install, or install without --network, opens a local browser Terms page and requires explicit human acceptance before installation proceeds"), "Install help should explain browser-based human Terms acceptance.");
  expect(stdout.includes("Sepolia and anvil installs do not require Terms acceptance"), "Install help should explain test-network Terms bypass.");
  expect(stdout.includes("Use --terminal-terms only when the local browser flow cannot be used for a Terms-gated install"), "Install help should explain the terminal fallback.");
  expect(stdout.includes("--json reports that browser-based interactive Terms acceptance is required for Terms-gated installs and does not install artifacts"), "Install help should explain JSON mode does not install for Terms-gated installs.");
  ...
  expect(stdout.includes("Requires an interactive terminal on mainnet because the output file contains secret-bearing viewing authority"), "Viewing-key export help should explain mainnet interactive confirmation.");
  expect(stdout.includes("Requires an interactive terminal on mainnet because the output file contains secret-bearing spending authority"), "Spending-key export help should explain mainnet interactive confirmation.");
  expect(stdout.includes("Sepolia and anvil exports do not require interactive confirmation"), "Export help should explain test-network confirmation bypass.");
  expect(stdout.includes("Use --export-evidence <PATH> to write a local full-note evidence ZIP for private-state-cli investigator; mainnet export requires interactive confirmation"), "Evidence export help should explain mainnet interactive confirmation.");
  expect(stdout.includes("Sepolia and anvil evidence exports do not require interactive confirmation"), "Evidence export help should explain test-network confirmation bypass.");
