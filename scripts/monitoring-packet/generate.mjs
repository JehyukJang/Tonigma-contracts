...
const PUBLIC_OUTPUT_DIR = process.env.TONIGMA_DOCS_MONITORING_OUTPUT_DIR
  ...
  : path.resolve(REPO_ROOT, "../tonigma-docs/publication/project-wide-policy/monitoring/data");
...
Public packet output:
  ../tonigma-docs/publication/project-wide-policy/monitoring/data/
...
async function buildOnchainSnapshot({ args, artifacts, rpcUrl }) {
  ...
  const [
    ...
    safeCall("L1TokenVault.JOIN_TOLL_BURN_ADDRESS", () => bridgeTokenVault.JOIN_TOLL_BURN_ADDRESS()),
    safeCall("L1TokenVault.channelOperationAbandonedAt", () => bridgeTokenVault.channelOperationAbandonedAt(channelId)),
    safeCall("ChannelManager.joinTollRefundCutoff1", () => channelManager.joinTollRefundCutoff1()),
    safeCall("ChannelManager.joinTollRefundCutoff2", () => channelManager.joinTollRefundCutoff2()),
    safeCall("ChannelManager.joinTollRefundCutoff3", () => channelManager.joinTollRefundCutoff3()),
    safeCall("ChannelManager.joinTollRefundBps1", () => channelManager.joinTollRefundBps1()),
    safeCall("ChannelManager.joinTollRefundBps2", () => channelManager.joinTollRefundBps2()),
    safeCall("ChannelManager.joinTollRefundBps3", () => channelManager.joinTollRefundBps3()),
    safeCall("ChannelManager.joinTollRefundBps4", () => channelManager.joinTollRefundBps4()),
  ...
  return {
    ...
    channel: {
      ...
      joinTollBurnAddress: joinTollBurnAddress.ok ? ethers.getAddress(joinTollBurnAddress.value) : null,
      joinTollRefundSchedule: {
        cutoff1Seconds: joinTollRefundCutoff1.ok ? joinTollRefundCutoff1.value.toString() : null,
        cutoff2Seconds: joinTollRefundCutoff2.ok ? joinTollRefundCutoff2.value.toString() : null,
        cutoff3Seconds: joinTollRefundCutoff3.ok ? joinTollRefundCutoff3.value.toString() : null,
        refundBps1: joinTollRefundBps1.ok ? joinTollRefundBps1.value.toString() : null,
        refundBps2: joinTollRefundBps2.ok ? joinTollRefundBps2.value.toString() : null,
        refundBps3: joinTollRefundBps3.ok ? joinTollRefundBps3.value.toString() : null,
        refundBps4: joinTollRefundBps4.ok ? joinTollRefundBps4.value.toString() : null,
      },
      operationAbandonedAt: channelOperationAbandonedAt.ok ? channelOperationAbandonedAt.value.toString() : null,
      operationAbandoned:
        channelOperationAbandonedAt.ok ? channelOperationAbandonedAt.value !== 0n : null,
    },
    bridge: {
...
function buildEventCoverage({ artifacts, onchain }) {
  ...
  return [
    ...
    eventEntry({
      ...
      observableAs: "StorageWriteObserved, ChannelExitRefunded, ChannelExitTollBurned, AssetsClaimed",
      artifactName: "bridgeTokenVault",
      contractAddress: addresses.bridgeTokenVault,
      abi: bridgeAbis.bridgeTokenVault,
    ...
    eventEntry({
      checklistItem: "channel exit toll burn-address transfer event",
      observableAs: "ChannelExitTollBurned",
      artifactName: "bridgeTokenVault",
      contractAddress: addresses.bridgeTokenVault,
      abi: bridgeAbis.bridgeTokenVault,
      eventName: "ChannelExitTollBurned",
      knownFromEvent: "L1 address, channel ID, non-refundable Join Toll amount, and burn-address transfer target.",
      notKnownFromEvent: "Complete private note path before channel exit.",
      monitoringMeaning: "Detects non-refundable Join Toll value transferred to the burn address during channel exit.",
    }),
    eventEntry({
      checklistItem: "channel operation abandonment event",
      observableAs: "ChannelOperationAbandoned plus channelOperationAbandonedAt getter",
      artifactName: "bridgeTokenVault",
      contractAddress: addresses.bridgeTokenVault,
      abi: bridgeAbis.bridgeTokenVault,
      eventName: "ChannelOperationAbandoned",
      knownFromEvent: "Channel ID, leader address, and abandonment timestamp.",
      notKnownFromEvent: "User-level blocking, note plaintext, private note provenance, custody authority, or recovery authority.",
      monitoringMeaning:
        "Detects public abandoned-operation status; abandoned channels reject new joins and channel deposits while preserving note activity, redeem, withdraw, and exit paths.",
    }),
    eventEntry({
...
function buildChannelPolicySnapshot({ args, artifacts, onchain }) {
  ...
  return {
    ...
    joinTollPolicy: {
      burnAddress: onchain.channel.joinTollBurnAddress,
      refundSchedule: onchain.channel.joinTollRefundSchedule,
      nonRefundablePortionHandling: "transferred-to-burn-address-on-future-channel-exit",
    },
    operationStatus: {
      abandoned: onchain.channel.operationAbandoned,
      abandonedAt: onchain.channel.operationAbandonedAt,
      abandonmentEffect:
        "New joins and channel deposits are rejected after abandonment; note activity, redeem, withdraw, and exit remain unrestricted by abandonment.",
    },
    ...
    storageLayoutHashAlgorithm: "keccak256(file bytes)",
    latestPolicyVersion,
    policyExplanationSource: "https://github.com/JehyukJang/Tonigma-docs/blob/main/publication/project-wide-policy/whitepaper.md#82-policy-surfaces",
  };
}
...
meaning of this matrix is described in \`https://github.com/JehyukJang/Tonigma-docs/blob/main/publication/project-wide-policy/whitepaper.md\`; this file records
...
\`https://github.com/JehyukJang/Tonigma-docs/blob/main/publication/project-wide-policy/whitepaper.md\`.
